...
export default {
  ...
  methods: {
    update() {
      ...
      this.achCountdown = new Decimal(1).div(gameSpeedupFactor).times(Achievements.timeToNextAutoAchieve);
      this.totalCountdown = new Decimal(1).div(gameSpeedupFactor).times(Achievements.period.times(Achievements.preReality.countWhere(a => !a.isUnlocked) - 1).add(Achievements.timeToNextAutoAchieve));
